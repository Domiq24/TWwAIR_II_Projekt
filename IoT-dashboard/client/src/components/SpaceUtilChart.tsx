...
function SpaceUtilChart() {
    ...
    useEffect(() => {
        ...
        .then(res => { setSpaces(res.data.map((space) => {return space.name})); })
        .catch(err => console.log(err))
    }, []);

    useEffect(() => {
        let url = "";
        if(mode === "dynamic") url = `http://localhost:3100/events/${spaces[spaceValue]}/data`;
        else if(mode === "cumulative") url = `http://localhost:3100/events/${spaces[spaceValue]}/data/${range}`;
        ...
        <>
            <Box sx={{textAlign: 'left', paddingLeft: '40px'}}>
                <FormControl variant="standard" size="small">
                    <InputLabel id='space_util_scope_label' sx={{color: 'white'}} >Scope</InputLabel>
                    <Select
                        labelId="space_util_scope_label"
                        value={range}
                        variant='standard'
                ...
                <FormControl variant="standard" size="small" sx={{marginLeft: '20px', flex: 1}}>
                    <InputLabel id='space_util_mode_label' sx={{color: 'white'}} >Mode</InputLabel>
                    <Select
                        labelId="space_util_mode_label"
                        value={mode}
                        variant='standard'
                ...
                <FormControl variant="standard" size="small" sx={{marginLeft: '20px', flex: 1}}>
                    <InputLabel id='space_util_space_label' sx={{color: 'white'}} >Space</InputLabel>
                    <Select
                        labelId="space_util_space_label"
                    ...
                    >
                        {spaces.map((space, index) => {
                            return (
                                <MenuItem value={index}>{space}</MenuItem>
                            )
                        })}
                    </Select>
                </FormControl>
